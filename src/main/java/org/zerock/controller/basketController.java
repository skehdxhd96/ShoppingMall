...
public class basketController {
	...
	@GetMapping(value = "/myPage/basket/{customer_code}/{page}", produces = { MediaType.APPLICATION_XML_VALUE,
			MediaType.APPLICATION_JSON_UTF8_VALUE })
	public ResponseEntity<basketPageVO> getList(@PathVariable("customer_code") Long customer_code,
	...
	@PostMapping(value = "/myPage/basket/new", consumes = "application/json", produces = { MediaType.TEXT_PLAIN_VALUE })
	public ResponseEntity<String> addBasketProduct(@RequestBody basketVO b) {

		int insertCount = bs.getBasketProduct(b);

		if (insertCount == 1) {
			return insertCount == 1 ? new ResponseEntity<>("Finish to add Product to Your Basket", HttpStatus.OK)
					: new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
		} else {
			return insertCount == 2 ? new ResponseEntity<>("Update to add Product to Your Basket", HttpStatus.OK)
					: new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}

	@DeleteMapping(value = "myPage/basket/delete/{customer_code}/{product_code}", produces = {
	...
	public ResponseEntity<String> RemoveBasketProduct(@PathVariable("customer_code") Long customer_code,
		...
		return RemoveBasket == 1 ? new ResponseEntity<>("Success Remove Product", HttpStatus.OK)
				: new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
}
